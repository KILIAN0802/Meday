    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
      ...
      <DialogContent dividers>
        {Object.keys(KEY_LABELS).map((key) => {
          if (person[key]) {
            const displayValue =
              key === 'role' ? roleMap[person[key]] || 'Không xác định' : person[key];
            return (
...
function StatusChip({ status }) {
  ...
  return (
    <Chip label={text} color={color} size="small" sx={{ fontWeight: 'bold' }} />
  );
}
...
export function AllMedicalRecords() {
  ...
  // Ánh xạ templateId sang tên bệnh án
  const templateMap = {
    16: 'Bệnh án cấp tính',
  ...
  const handleReview = (recordId) => {
    console.log(`Xem lại bệnh án có ID: ${recordId}`);
  };
  const handleChangePage = (event, newPage) => {
    setPage(newPage);
  };

  // Lấy toàn bộ appointment (không filter status)
  useEffect(() => {
    const fetchAllAppointmentData = async () => {
      setLoading(true);
      setError(null);
      try {
        const appointmentResponse = await getAppointment(); // không truyền status
  ...
  // Lấy chi tiết record cho từng page
  useEffect(() => {
    ...
    const fetchDetailsForCurrentPage = async () => {
      ...
      try {
        ...
        if (idsForCurrentPage.length > 0) {
          const detailPromises = idsForCurrentPage.map((id) =>
            getMedicalRecordeById(id)
          );
  ...
  return (
    <Container maxWidth="xl">
      <Typography variant="h4" sx={{ mb: 5 }}>
        Danh sách Tất cả Bệnh án
      </Typography>
      <Card>
        {error && (
          <Typography color="error" sx={{ px: 3 }}>
            Lỗi: {error}
          </Typography>
        )}
              ...
              {loading ? (
                ...
                medicalRecords.map((row) => (
                  <TableRow key={row.id} hover>
                    ...
                    <TableCell>
                      <Typography
                        onClick={() => handleViewDetails(row.patient)}
                        variant="body2"
                        sx={{
                          color: 'primary.main',
                          cursor: 'pointer',
                    ...
                    <TableCell>
                      {templateMap[row.templateId] || 'N/A'}
                    </TableCell>
                    <TableCell>{row.appointment?.reason || 'N/A'}</TableCell>
                    <TableCell align="center">
                      <StatusChip
                        status={row.appointment?.status || 'UNKNOWN'}
                      />
                    </TableCell>
                  </TableRow>
                ))
